...
//            href: url.isFileURL ? url.lastPathComponent : url.absoluteString,
        ...
        if (try! BooksDatabase.shared.books.insert(book: book, allowDuplicate: true)) != nil {
            ...
            let duplicatePublicationAlert = UIAlertController(
                title: NSLocalizedString("library_duplicate_alert_title", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Title of the import confirmation alert when the publication already exists in the library"),
                message: NSLocalizedString("library_duplicate_alert_message", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Message of the import confirmation alert when the publication already exists in the library"),
                preferredStyle: UIAlertController.Style.alert
            )
            let addAction = UIAlertAction(title: NSLocalizedString("add_button", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Confirmation button to import a duplicated publication"), style: .default, handler: { alert in
                if (try! BooksDatabase.shared.books.insert(book: book, allowDuplicate: true)) != nil {
            ...
            let cancelAction = UIAlertAction(title: NSLocalizedString("cancel_button", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Cancel the confirmation alert"), style: .cancel, handler: { alert in
                try? FileManager.default.removeItem(at: url)
                self.delegate?.reloadLibrary(with: downloadTask, canceled: true)
                return
            })
    ...
    func parsePublication(atPath path: String) -> PubBox? {
        let path: String = {
            ...
            if path.hasPrefix("file://") {
                return String(path.suffix(path.count - 7))
            }
