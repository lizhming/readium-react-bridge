...
@objc(OutlineTableViewController)
            ...
            if let bookmark = bookmarksDataSource?.bookmark(at: indexPath.item) {
                ...
                cell.detailTextLabel?.text = {
                    if let position = bookmark.locator.locations?.position {
                        return String(format: NSLocalizedString("reader_outline_position_label", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Outline bookmark label when the position is available"), position)
                    } else if let progression = bookmark.locator.locations?.progression {
                        return String(format: NSLocalizedString("reader_outline_progression_label", bundle: Bundle(path: Bundle.main.path(forResource: "RNReadiumReactBridgeBundle", ofType: "bundle")!)!, comment: "Outline bookmark label when the progression is available"), progression * 100)
                    } else {
                        return nil
                    }
                }()
